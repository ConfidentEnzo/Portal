      ...
      <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
          <ul class="nav navbar-nav side-nav">
            ...
            <li class="active"><a href="cuisine.html"><i class="fa fa-wrench"></i>最近新菜</a></li>
...
<div class="row">
                ...
                <div class="bs-example" data-example-id="panel-without-body-with-table">
                    <div class="panel panel-default">
                        ...
                        <table class="table">
                          ...
                          <tbody>
                            <?php if(is_array($cuisine_list)): foreach($cuisine_list as $key=>$item): ?><tr>
                                  <td><?php echo ($item["id"]); ?></td>
                                  <td><?php echo ($item["cuisine_name"]); ?></td>
                                  <td><img src="/yingsen/Public/uploads/<?php echo ($item["cuisine_img"]); ?>" class="img-responsive" alt="picture1"></td>
                                  <td><?php echo ($item["cuisine_intro"]); ?></td>
                                  <td> 
                                    <?php if($item["is_show"] == 1): ?><button type="privilege" class="btn btn-default" onclick="not_show_cuisine(<?php echo ($item["id"]); ?>)" style="color:red;">取消显示</button>
                                    <?php else: ?>
                                      <button type="privilege" class="btn btn-default" onclick="show_cuisine(<?php echo ($item["id"]); ?>)"  >主页显示</button><?php endif; ?>
                                    <br/>
                                    <button type="delete" class="btn btn-default" onclick="del_cuisine(<?php echo ($item["id"]); ?>)">删除</button>
                                  </td>
    ...
    <script type="text/javascript">
      function del_cuisine(id){
        if (!confirm("确认要删除？")) {
          return;
        }
        var url = "<?php echo U('cuisine/delcuisine');?>";
        $.post(url,{
          cuisine_id:id
        },function(response){
      ...
      function show_cuisine(id){
        var url = "<?php echo U('cuisine/showcuisine');?>";
        $.post(url,{
          cuisine_id:id
        },function(response){
          console.log(response);
      ...
      function not_show_cuisine(id){
        var url = "<?php echo U('cuisine/notshowcuisine');?>";
        $.post(url,{
          cuisine_id:id
        },function(response){
          console.log(response);
